...
def build_pdx_gex(gdsc_path, pdx_path, allowed_samples=None):
    ...
    LOGGER.info(
        ...
        len(pdx.index),
        len(common_genes),
        len(gdsc.columns) - len(common_genes),
    )
    ...
    if allowed_samples is not None:
        ...
        if missing_samples:
            LOGGER.warning(
                "Skipping %d models without expression values",
                len(missing_samples),
            )
        aligned = aligned.loc[aligned.index.intersection(allowed_samples)]
        aligned = aligned.sort_index(axis=0)

    # Pad to full GDSC gene space with zeros for missing genes
...
def parse_args():
    ...
    parser.add_argument("--out-gex", type=Path, default=Path("../data/PDX_unlabeled_gex.csv"), help="Output path for aligned expression")
    parser.add_argument("--out-info", type=Path, default=Path("../data/PDX_unlabeled_info.csv"), help="Output path for metadata")
...
def main() -> None:
    ...
    if missing_metadata:
        LOGGER.warning(
            "Dropping %d expression samples without metadata",
    ...
    if metadata_only:
        LOGGER.warning(
            "Excluding %d metadata entries without expression",
            len(metadata_only),
        )
